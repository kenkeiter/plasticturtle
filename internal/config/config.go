...
// should not have to run plasticturtle allow five times.
...
// It is exported so an interactive front end (plasticturtle init) can validate a domain as
...
// Template renders a commented .plasticturtle for plasticturtle init. The comments are
...
func Template(c *Config, generatedAt time.Time) ([]byte, error) {
	...
	// plasticturtle init builds the Config from its picker and need not know the schema
	...
	b.WriteString("# Checked into the repo, human-edited. After any change, run `plasticturtle allow`:\n")
	b.WriteString("# this file is inert until its exact contents are trusted.\n")
	if !generatedAt.IsZero() {
		b.WriteString("# Generated by `plasticturtle init` on " + generatedAt.UTC().Format(time.RFC3339) + ".\n")
	}
	...
	b.WriteString("# the firewall shim installed once; run `plasticturtle setup` if a boot complains.\n")
