...
| F6 | `chooseRemoteAddr` probed `vmIp:<port>` from the **host's** stack at tunnel-setup time, when nothing is listening on either candidate — so it answered "no" every time and hardcoded loopback. The fallback of items 3/17 was dead code, and a guest service bound only to the external interface was unreachable. | `Client.ForwardAny` (additive; `Forward` delegates to it) takes candidates in preference order, and the tunnel latches whichever answers on its **first real connection** — the earliest moment the question has a true answer. A dead candidate costs one dial per tunnel, not per connection, which was item 17's objection to per-connection fallback. **This amends items 3 and 17.** |
| F11 | `pt _supervise` booted from its stdin params without consulting `trust.json`, so the trust decision was made in exactly one place. | The supervisor now checks its snapshotted `ConfigHash` against the trust database **before cloning**. It checks the hash rather than re-reading the file, because the config is deliberately snapshotted at creation (§6.2) and the file may legitimately have changed since — re-reading would refuse boots the design allows. Still not a security boundary (anyone who can run `_supervise` can run `tart`), but trust is now layered rather than singular. |
| F15 | The exclusive project lock was held across `tart stop`/`tart delete` with no deadline — the longest hold in the system and the only one whose duration pt does not control. One wedged `tart delete` blocked every other invocation for that project indefinitely. | The reclaim is bounded by `ptcfg.ReclaimTimeout` (60s). On expiry the lock is released and the next GC pass retries. Tested by asserting the deadline the subprocess calls receive, rather than by waiting a minute for it. |
